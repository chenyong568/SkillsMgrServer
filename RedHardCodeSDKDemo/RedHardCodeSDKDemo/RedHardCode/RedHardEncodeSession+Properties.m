...
@implementation RedHardCode (EncoderProperties)
...
- (NSTimeInterval)ExpectedFrameRate
{
    NSNumber *value = [self valueForProperty:(__bridge NSString *)kVTCompressionPropertyKey_ExpectedFrameRate error:nil];
    return value.doubleValue;
}

- (BOOL)setExpectedFrameRate:(NSTimeInterval)expectedFrameRate error:(NSError **)error
{
    return [self setValue:@(expectedFrameRate) forProperty:(__bridge NSString *)kVTCompressionPropertyKey_ExpectedFrameRate error:error];
}
