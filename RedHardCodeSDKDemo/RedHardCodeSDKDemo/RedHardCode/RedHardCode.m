...
NSString * const VTErrorDomain = @"RedVideoToolboxError";
...
- (id)valueForProperty:(NSString *)property error:(NSError **)outError
{
    ...
    OSStatus status = VTSessionCopyProperty(_encoderSession, (__bridge CFStringRef)property, NULL, &value);
    if(status != noErr)
    {
        NSError *error = [self videoToolboxErrorWithStatus:status];
        if(outError != nil)
        {
            *outError = error;
        }
...
{
    OSStatus status = VTSessionSetProperty(_encoderSession, (__bridge CFStringRef)property, (__bridge CFTypeRef)value);
    if(status != noErr)
    {
        NSError *error = [self videoToolboxErrorWithStatus:status];
        if(outError != nil)
...
//错误描述
NSString * descriptionWithStatus(OSStatus status)
{
    switch(status)
    {
...
- (NSError*)videoToolboxErrorWithStatus:(OSStatus)status
{
    ...
    return [NSError errorWithDomain:VTErrorDomain code:(NSInteger)status userInfo:userInfo];
}
@end
