...
#import "RedHardEncodeSession+Properties.h"
...
@property (weak, nonatomic) IBOutlet UILabel *gopDur;
...
- (IBAction)frameRateChanged:(id)sender {
    ...
    //不起作用（暂时不用）
//    NSError* retError;
//    [hardcode  setExpectedFrameRate:frameRate error:&retError];
}

- (IBAction)bitRateChanged:(UISlider *)sender {
    NSError *retError;
    int nValue = 480*640*(int)sender.value;
    _bitRate.text = [NSString stringWithFormat:@"%d",nValue];
    [hardcode setAverageBitrate:nValue error:&retError];
}

- (IBAction)gopChanged:(id)sender {
    NSError *retError;
    UISlider* slider = (UISlider*)sender;
    float fValue = slider.value;
    _gopDur.text = [NSString stringWithFormat:@"%f",fValue];
    [hardcode setMaxKeyframeIntervalDuration:fValue  error:&retError];
}
